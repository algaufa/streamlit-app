...
if not df_hist.empty:
    dates = sorted(df_hist["Дата"].unique())
    if len(dates) >= 2:
        last_total = df_hist[df_hist["Дата"] == dates[-1]]["Сумма_руб"].sum()
        prev_total = df_hist[df_hist["Дата"] == dates[-2]]["Сумма_руб"].sum()
        delta = last_total - prev_total
        delta_str = f"{delta:+.2f}"
        st.markdown(f"### Последний расчёт: {last_total:.2f} ({delta_str}) {dates[-1]}")
    else:
        last_total = df_hist[df_hist["Дата"] == dates[-1]]["Сумма_руб"].sum()
        st.markdown(f"### Последний расчёт: {last_total:.2f} {dates[-1]}")
else:
...
with st.sidebar:
    ...
    tab1, tab2, tab3 = st.tabs(["⚙️ Услуги", "📅 Тарифы", "💾 Данные"])
        ...
        if available_dates:
            ...
            with col2:
                ...
                if st.session_state.confirm_delete_date == date_to_delete:
                    ...
                    col_yes, col_no = st.columns(2)
                    with col_yes:
                        if st.button("✅ Да, удалить", key="confirm_delete_date_yes"):
                            ...
                            st.rerun()
                    with col_no:
                        if st.button("❌ Отмена", key="confirm_delete_date_no"):
                            st.session_state.confirm_delete_date = None
                            st.rerun()
            ...
            if not df_detail.empty:
                ...
                for srv in unique_services:
                    ...
                    if not srv_df.empty:
                        ...
                        current = float(row_data["Показания"])
                        consumption = float(row_data["Расход"]) if not pd.isna(row_data["Расход"]) else 0.0
